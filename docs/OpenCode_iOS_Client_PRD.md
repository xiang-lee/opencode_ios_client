...
- `tool` — 工具调用卡片，显示工具名称和状态（pending → running → completed/error）。状态为 running 时显示 spinner。completed 时可点击展开查看元数据（如文件路径、命令输出）
- `step-start` / `step-finish` — 渲染为步骤分隔线，显示 token 用量和成本
- `patch` — 文件变更摘要卡片，显示修改的文件列表，点击跳转到 Files Tab 的 diff 视图

**流式更新**：SSE 推送 `message.part.updated` 事件时带有 `delta` 字段（增量文本）。客户端实时追加到对应 Part 的文本末尾，实现打字机效果。使用 `messageID` + `partID` 定位到具体的 Part 进行更新。
